// File: src-tauri/src/gnn/parser_cpp.rs
// Purpose: tree-sitter C++ parser for GNN
...
/// Parse a C++ file and extract nodes and edges
...
fn extract_nodes_and_edges(
    ...
    fn visit_node(
        ...
        match node.kind() {
            ...
            "namespace_definition" => {
                if let Some(name_node) = node.child_by_field_name("name") {
                    if let Ok(name) = name_node.utf8_text(code.as_bytes()) {
                        let start = node.start_position();
                        let end = node.end_position();
                        let id = format!("{}::namespace::{}::{}", file_path, name, start.row);
            ...
            "preproc_include" => {
                if let Ok(include_text) = node.utf8_text(code.as_bytes()) {
                    let include_path = include_text
                        .trim_start_matches("#include ")
                        .trim()
                        .trim_matches(|c| c == '<' || c == '>' || c == '"');
                    let start = node.start_position();
                    let id = format!("{}::include::{}::{}", file_path, include_path, start.row);
                    ...
                    edges.push(CodeEdge {
                        ...
                        source_id: id,
                        target_id: include_path.to_string(),
                    });
                }
            }
            _ => {}
...
"#;
        let path = PathBuf::from("/test/calc.cpp");
        let (nodes, _edges) = parse_cpp_file(code, &path).unwrap();
...
"#;
        let path = PathBuf::from("/test/point.cpp");
        let (nodes, _edges) = parse_cpp_file(code, &path).unwrap();
    ...
    #[test]
    fn test_parse_cpp_include() {
        let code = r#"
#include <iostream>
#include <vector>
#include "myheader.hpp"
"#;
        let path = PathBuf::from("/test/main.cpp");
        let (nodes, edges) = parse_cpp_file(code, &path).unwrap();

        let include_nodes: Vec<&CodeNode> = nodes
            .iter()
            .filter(|n| matches!(n.node_type, NodeType::Import))
            .collect();

        assert_eq!(include_nodes.len(), 3);
