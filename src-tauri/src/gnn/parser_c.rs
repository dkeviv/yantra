// File: src-tauri/src/gnn/parser_c.rs
...
"#;
        let path = PathBuf::from("/test/calc.c");
        let (nodes, _edges) = parse_c_file(code, &path).unwrap();
...
"#;
        let path = PathBuf::from("/test/point.c");
        let (nodes, _edges) = parse_c_file(code, &path).unwrap();
...
#include "myheader.h"
"#;
        let path = PathBuf::from("/test/main.c");
        let (nodes, edges) = parse_c_file(code, &path).unwrap();
