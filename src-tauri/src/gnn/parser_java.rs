// File: src-tauri/src/gnn/parser_java.rs
// Purpose: tree-sitter Java parser for GNN
...
"#;
        let path = PathBuf::from("/test/Calculator.java");
        let (nodes, _edges) = parse_java_file(code, &path).unwrap();
...
"#;
        let path = PathBuf::from("/test/Point.java");
        let (nodes, _edges) = parse_java_file(code, &path).unwrap();
...
public class Main {}
"#;
        let path = PathBuf::from("/test/Main.java");
        let (nodes, edges) = parse_java_file(code, &path).unwrap();
