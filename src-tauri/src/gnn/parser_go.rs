// File: src-tauri/src/gnn/parser_go.rs
// Purpose: tree-sitter Go parser for GNN
...
/// Parse a Go file and extract nodes and edges
...
fn extract_nodes_and_edges(
    ...
    fn visit_node(
        ...
        match node.kind() {
            ...
            "type_declaration" => {
                if let Some(spec_node) = node.child_by_field_name("type") {
                ...
                for child in node.children(&mut child_cursor) {
                    if child.kind() == "import_spec" {
                        if let Some(path_node) = child.child_by_field_name("path") {
                            if let Ok(import_path) = path_node.utf8_text(code.as_bytes()) {
                                let import_path = import_path.trim_matches('"');
                                let start = child.start_position();
                                let id = format!("{}::import::{}::{}", file_path, import_path, start.row);
                                ...
                                edges.push(CodeEdge {
                                    ...
                                    source_id: id,
                                    target_id: import_path.to_string(),
                                });
                            }
                        }
                    }
...
"#;
        let path = PathBuf::from("/test/main.go");
        let (nodes, _edges) = parse_go_file(code, &path).unwrap();
...
"#;
        let path = PathBuf::from("/test/point.go");
        let (nodes, _edges) = parse_go_file(code, &path).unwrap();

        let type_nodes: Vec<&CodeNode> = nodes
            .iter()
            .filter(|n| matches!(n.node_type, NodeType::Class))
            .collect();

        assert_eq!(type_nodes.len(), 1);
        assert_eq!(type_nodes[0].name, "Point");
    }

    #[test]
    fn test_parse_go_import() {
        let code = r#"
package main
...
"#;
        let path = PathBuf::from("/test/imports.go");
        let (nodes, edges) = parse_go_file(code, &path).unwrap();
