// File: src-tauri/src/gnn/parser_ruby.rs
// Purpose: tree-sitter Ruby parser for GNN
...
/// Parse a Ruby file and extract nodes and edges
        ...
        match node.kind() {
            ...
            "module" => {
                if let Some(name_node) = node.child_by_field_name("name") {
                    if let Ok(name) = name_node.utf8_text(code.as_bytes()) {
                        let start = node.start_position();
                        let end = node.end_position();
                        let id = format!("{}::module::{}::{}", file_path, name, start.row);
            ...
            "call" => {
                if let Some(method_node) = node.child_by_field_name("method") {
                    if let Ok(method_name) = method_node.utf8_text(code.as_bytes()) {
                        if method_name == "require" || method_name == "require_relative" {
                            // Extract the require argument
                            if let Some(args_node) = node.child_by_field_name("arguments") {
                                if let Ok(args_text) = args_node.utf8_text(code.as_bytes()) {
                                    let require_path = args_text
                                        .trim()
                                        .trim_start_matches('(')
                                        .trim_end_matches(')')
                                        .trim_matches(|c| c == '\'' || c == '"');
                                    ...
                                    let id = format!(
                                        "{}::require::{}::{}",
                                        file_path, require_path, start.row
                                    );
                                    ...
                                    edges.push(CodeEdge {
                                        ...
                                        source_id: id,
                                        target_id: require_path.to_string(),
                                    });
                                }
                            }
                        }
...
"#;
        let path = PathBuf::from("/test/calc.rb");
        let (nodes, _edges) = parse_ruby_file(code, &path).unwrap();

        let method_nodes: Vec<&CodeNode> = nodes
            .iter()
            .filter(|n| matches!(n.node_type, NodeType::Function))
            .collect();

        assert_eq!(method_nodes.len(), 1);
        assert_eq!(method_nodes[0].name, "add");
    }
...
"#;
        let path = PathBuf::from("/test/point.rb");
        let (nodes, _edges) = parse_ruby_file(code, &path).unwrap();
    ...
    #[test]
    fn test_parse_ruby_require() {
        let code = r#"
require 'json'
require_relative 'helpers'
"#;
        let path = PathBuf::from("/test/main.rb");
        let (nodes, edges) = parse_ruby_file(code, &path).unwrap();

        let require_nodes: Vec<&CodeNode> = nodes
            .iter()
            .filter(|n| matches!(n.node_type, NodeType::Import))
            .collect();

        assert_eq!(require_nodes.len(), 2);
