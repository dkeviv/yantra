// File: src-tauri/src/gnn/parser_php.rs
// Purpose: tree-sitter PHP parser for GNN
// Dependencies: tree-sitter, tree-sitter-php
...
/// Parse a PHP file and extract nodes and edges
            ...
            "namespace_use_declaration" => {
                ...
                for child in node.children(&mut child_cursor) {
                    if child.kind() == "namespace_use_clause" {
                        if let Ok(use_text) = child.utf8_text(code.as_bytes()) {
                            let start = child.start_position();
                            let id = format!("{}::use::{}::{}", file_path, use_text, start.row);
...
?>
"#;
        let path = PathBuf::from("/test/calc.php");
        let (nodes, _edges) = parse_php_file(code, &path).unwrap();
...
?>
"#;
        let path = PathBuf::from("/test/point.php");
        let (nodes, _edges) = parse_php_file(code, &path).unwrap();
...
?>
"#;
        let path = PathBuf::from("/test/main.php");
        let (nodes, edges) = parse_php_file(code, &path).unwrap();

        let use_nodes: Vec<&CodeNode> = nodes
            .iter()
