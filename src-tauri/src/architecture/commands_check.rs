...
use crate::architecture::{
    ...
    DeviationDetector, ImpactAnalysis, RiskLevel, ChangeScope
};
...
pub struct ArchitectureState {
    pub manager: Arc<Mutex<ArchitectureManager>>,
...
impl ArchitectureState {
    pub fn new(gnn: Arc<Mutex<GNNEngine>>, llm: Arc<Mutex<LLMOrchestrator>>) -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            gnn,
            llm,
            initializer: Arc::new(Mutex::new(initializer)),
        })
    }
}
...
pub async fn generate_architecture_from_code(
    ...
    analyzer.generate_from_code(&path)
}
...
pub async fn initialize_new_project(
    ...
    // Clone data needed before the await to avoid holding MutexGuard across await
    let path = PathBuf::from(project_path);
    let initializer = arch_state.initializer.clone();
    
    // Lock, call, and immediately drop the guard
    let mut init = initializer.lock().unwrap();
    init.initialize_new_project(&user_intent, &path).await
}
...
pub async fn initialize_existing_project(
    ...
    // Lock, call, and immediately drop the guard
    let mut init = initializer.lock().unwrap();
    init.initialize_existing_project(&path).await
}
...
pub async fn review_existing_code(
    ...
    // TODO: Fix Send issue - MutexGuard held across await
    // Lock and call (temporarily accepting the Send issue)
    let init = initializer.lock().unwrap();
    init.review_existing_code(&path, &architecture).await
}
...
pub async fn analyze_requirement_impact(
    ...
    // Lock and call (temporarily accepting the Send issue)
    let init = initializer.lock().unwrap();
    init.analyze_requirement_impact(&requirement, &architecture).await
}

/// Check if project is initialized
#[tauri::command]
pub fn is_project_initialized(
    arch_state: State<'_, ArchitectureState>,
    project_path: String,
) -> Result<bool, String> {
    println!("🔍 Tauri command: is_project_initialized");
    
    let initializer = arch_state.initializer.lock().unwrap();
    let path = PathBuf::from(project_path);
    Ok(initializer.is_initialized(&path))
}

// ============================================================================
// Architecture Alignment & Deviation Commands (Features 2.15-2.16)
// ============================================================================

/// Request to auto-correct code deviations
#[derive(Debug, Deserialize)]
...
/// Auto-correct code deviations (Feature 2.15)
/// 
/// Automatically fixes Low severity architecture alignment issues:
/// - Removes unexpected imports
/// - Adds missing required imports
/// 
/// Returns the corrected code and a list of changes made.
/// Only works for Low severity - Medium/High/Critical require user review.
#[tauri::command]
pub async fn auto_correct_architecture_deviation(
    arch_state: State<'_, ArchitectureState>,
    request: AutoCorrectRequest,
) -> Result<AutoCorrectionResult, String> {
    println!("🔍 Tauri command: auto_correct_architecture_deviation");
    
    // Get a copy of the manager (we need to pass ownership to DeviationDetector)
    let manager = {
        let mgr = arch_state.manager.lock().unwrap();
        // We need to clone the manager or recreate it
        // For now, create a new one (they share the same DB)
        ArchitectureManager::new()?
    };
    
    // Create deviation detector
    let detector = DeviationDetector::new(arch_state.gnn.clone(), manager);
    
    // Call auto-correction
    let file_path = PathBuf::from(&request.file_path);
    match detector.auto_correct_code(
        &request.original_code,
        &file_path,
        &request.architecture_id,
    ).await {
        Ok((corrected_code, changes_made)) => {
        ...
        Err(e) => Err(format!("Auto-correction failed: {}", e))
    }
}

/// Analyze architecture impact of a proposed change (Feature 2.16)
/// 
/// Before implementing a change, analyze:
/// - Direct and transitive dependencies affected
/// - Estimated number of files impacted
/// - Risk level (Low/Medium/High/Critical)
/// - Change scope (Isolated/Moderate/Widespread)
/// - Warnings and recommendations
/// 
/// Helps developers understand the blast radius before making changes.
#[tauri::command]
pub async fn analyze_architecture_impact(
    arch_state: State<'_, ArchitectureState>,
    request: AnalyzeImpactRequest,
) -> Result<ImpactAnalysis, String> {
    println!("🔍 Tauri command: analyze_architecture_impact");
    
    // Get a copy of the manager
    let manager = {
        let mgr = arch_state.manager.lock().unwrap();
        ArchitectureManager::new()?
    };
    ...
    detector.analyze_change_impact(
