...
import { architectureState, exportArchitecture } from '../../stores/architectureStore';
  ...
  return (
    <div class="flex flex-col h-full" style={{ 'background-color': 'var(--bg-primary)' }}>
      ...
      <div class="flex-1 relative">
        ...
        <Show when={architectureState.current && !architectureState.isLoading}>
          <div
            class="absolute bottom-4 right-4 backdrop-blur border rounded-lg shadow-lg px-3 py-2 text-xs"
            style={{
          ...
          >
            <div class="flex items-center gap-2">
              ...
              <span>
                Updated:{' '}
                {new Date(architectureState.current?.updated_at || Date.now()).toLocaleTimeString()}
              </span>
            </div>
          </div>
        </Show>
