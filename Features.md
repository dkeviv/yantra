...
## 🎉 Major UI & System Updates (November 23, 2025)

### ✅ Three-Column Layout Redesign
- **Left Column (20%):** File Tree + Agent Status (at bottom)
- **Center Column (45%):** Chat Panel (full height for conversations)
- **Right Column (35%):** Code Editor + Multi-Terminal (stacked vertically)
- **Benefits:** Terminal no longer takes space from all panels, better space utilization

### ✅ Native View Menu (macOS/Windows/Linux)
- **File Menu:** Standard Copy, Paste, Quit
- **View Menu:**
  - Toggle File Tree (Cmd+B / Ctrl+B)
  - Toggle Code Editor (Cmd+E / Ctrl+E)
  - Toggle Terminal (Cmd+` / Ctrl+`)
  - Show Dependencies (Cmd+D / Ctrl+D)
  - Reset Layout
- **Help Menu:** Documentation, About
- **Implementation:** Native Tauri menu system with keyboard shortcuts

### ✅ Multi-Terminal System (VSCode-style)
**Description:** Intelligent multi-terminal management with automatic command routing

**Key Features:**
1. **Multiple Terminal Instances:** Create unlimited terminals (recommended max: 10)
2. **Status Tracking:** Each terminal shows idle/busy/error state with visual indicators
3. **Intelligent Command Routing:**
   ...
   - Prefers idle terminals over creating new ones
4. **Terminal Stats Dashboard:**
   - Total terminals count
   - Idle terminals (green)
   - Busy terminals (yellow, animated)
   - Error terminals (red)
5. **Individual Terminal Control:**
   - Switch between terminals via tabs
   - Close terminals (minimum 1 required)
   - Clear terminal output
   - Rename terminals

**User Benefits:**
- **No Command Conflicts:** Multiple long-running processes simultaneously
- **Agent Intelligence:** AI automatically finds available terminals
- **Visual Clarity:** Status indicators show exactly what's running where
- **Developer Workflow:** Just like VSCode terminal experience

**Use Cases:**

**Use Case 1: Parallel Development**
```
Scenario: Running dev server, tests, and git operations simultaneously

Terminal 1 (Busy): npm run dev (long-running)
Terminal 2 (Idle): Available for git commands
Terminal 3 (Busy): pytest --watch (long-running)

Agent needs to run: git status
→ Intelligently routes to Terminal 2 (idle)
→ Doesn't interrupt dev server or tests
```

**Use Case 2: Build + Test Workflow**
```
Scenario: User triggers "build and test"

Agent:
1. Checks terminal availability: 2 idle terminals found
2. Terminal 1: Executes npm run build
3. Waits for build completion
4. Terminal 2: Executes npm test
5. Both outputs visible in separate tabs
```

**Technical Details:**
- **Terminal Manager:** `terminalStore.ts` with state management
- **Methods:**
  - `findIdleTerminal()` - Finds available terminal
  - `executeCommand(cmd, preferredId?)` - Smart routing
  - `completeCommand(id, output, success)` - Updates status
  - `canExecuteCommand()` - Pre-check availability
  - `getStats()` - Terminal statistics
- **State Tracking:**
  - `id`, `name`, `status`, `currentCommand`
  - `output[]`, `createdAt`, `lastUsed`

### ✅ Multi-File Tab System (VSCode-style)
**Description:** Open and manage multiple files simultaneously with tab navigation

**Key Features:**
1. **File Tabs:** Visual tabs showing all open files
2. **Active File Highlighting:** Current file has distinct styling
3. **Close Buttons:** X button on each tab
4. **File Path Display:** Header shows full path of active file
5. **Tab Switching:** Click any tab to switch to that file
6. **Automatic Opening:** Click file in tree → Opens in new tab or switches to existing

**User Benefits:**
- **Rapid Navigation:** Switch between files without losing context
- **Multiple File Editing:** Work on several files simultaneously
- **Visual Context:** See all open files at a glance
- **Familiar UX:** Same behavior as VSCode

### ✅ Recursive File Tree Navigation
**Description:** Fully functional file tree with folder expansion and file loading

**Key Features:**
1. **Recursive Expansion:** Click folders to expand/collapse
2. **Lazy Loading:** Subfolders loaded on-demand (performance)
3. **Smart Sorting:** Directories first, then alphabetical
4. **File Type Icons:** Visual indicators for Python, JS, JSON, etc.
5. **Depth Indentation:** Visual hierarchy shows nesting levels
6. **State Management:** Remembers expanded/collapsed state

**Technical Implementation:**
- **Tree Structure:** `TreeNode` with `children[]`, `isExpanded`
- **Rendering:** Recursive `renderTree()` function
- **Path Tracking:** Maintains path for nested navigation
- **Performance:** Only loads directories when user expands them

### ✅ View Routing in Code Panel
**Description:** Switch between different views in the code panel (column 3)

**Available Views:**
1. **Code Editor** (Default) - Monaco editor with file tabs
2. **Dependencies** (Placeholder) - Dependency graph (coming soon)

**Features:**
- View selector tabs at top of code panel
- Active view highlighted
- All views render in same space (column 3)
- Keyboard shortcut: Cmd+D to show dependencies

### ✅ Git MCP Integration (Full Suite)
**Description:** Complete Git operations available to agent via Model Context Protocol

**Implemented Operations:**
1. `git_status` - Get repository status
2. `git_add` - Stage files
3. `git_commit` - Commit with message
4. `git_diff` - View changes
5. `git_log` - Commit history
6. `git_branch_list` - List branches
7. `git_current_branch` - Active branch name
8. `git_checkout` - Switch branches
9. `git_pull` - Pull from remote
10. `git_push` - Push to remote

**Agent Capabilities:**
- Programmatic Git operations (no manual commands needed)
...
- **Backend:** `src-tauri/src/git/mcp.rs` with all Git operations
- **Tauri Commands:** 10 commands exposed to frontend
- **Frontend API:** `src-ui/utils/git.ts` with TypeScript wrappers
- **Agent Integration:** Orchestrator can call Git operations automatically
