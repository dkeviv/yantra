...
- ✅ **Architecture Reconsideration** - After user feedback, decided to move architecture into GNN for instant alignment checks
- ✅ **Storage Simplification** - Everything code-related now in single `.yantra/graph.db`: Dependencies + Templates + Known Issues + Architecture
- ✅ **Cloud Sync Unified** - Single PostgreSQL sync system instead of separate architecture sync mechanism
- ✅ **Alignment Performance** - Architecture violations now detected via single graph query (<100ms) instead of querying 2 DBs
- ✅ **Node Types Extended** - Component/Service/Layer nodes to be added to GNN (migration post-MVP)
- ✅ **Three Major Decisions**:
  1. Templates → GNN (semantic search integration)
  2. Known Issues → GNN (graph relationships with code)
  3. Architecture → GNN (instant alignment, single sync)
- ✅ **Documentation Updated**:
  - Decision_Log.md: Three detailed decision entries with full rationale
  - Specifications.md: Storage table updated, database clarification table updated, unified storage explanation
  - Migration notes added to architecture storage section
- 📊 **Impact**: Simpler architecture, faster alignment checks, single cloud sync point, easier maintenance
- 🎯 **Next Steps**: Extend NodeType/EdgeType enums, migrate architecture module (post-MVP, 2-3 days)
- 🔄 **Migration Plan**: `.yantra/architecture.db` → Component/Service/Layer nodes in `.yantra/graph.db`

**PREVIOUS: Comprehensive Agentic Capabilities Framework Added (Dec 1, 2025 - 5:00 PM)**
...
| File                                                        | Status        | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Dependencies                                                | Last Updated |
| ----------------------------------------------------------- | ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- | ------------ |
| `docs/Dependency_Workflow_Complete.md`                      | ✅ **NEW**    | **COMPLETE DEPENDENCY WORKFLOW**: End-to-end flow for new & existing projects with GNN integration. Covers: (1) New project creation (DependencyAssessment → EnvironmentSetup → Graph creation), (2) Existing project modification (incremental updates, breaking change analysis), (3) Drift detection & resolution. Includes state machine integration, GNN query API, error handling, performance targets. 330 lines with detailed Rust code examples.                                     | .github/Specifications.md, Dependency_Detection_Strategy.md | Dec 5, 2025  |
| `docs/Dependency_Detection_Strategy.md`                     | ✅ **NEW**    | **HYBRID DEPENDENCY DETECTION**: Explains lock files vs runtime inspection vs manifest files. Rationale for using `pip show`/`npm list` during development (exact versions) vs lock files (production/CI). Documents 3 sources of truth with pros/cons, decision tree, real-world scenarios, performance comparison. Justifies spec change from "skip runtime inspection" to "runtime inspection as primary during dev". 330 lines.                                                           | .github/Specifications.md                                   | Dec 5, 2025  |
